...
void EventBrowsingandBooking::searchEventsByKeyword(const QString& keyword) {
    ...
    // Ensure the 'events' vector is a class-level member variable populated earlier
    const std::vector<Event> events; // Adjusted to use the correct member variable
...
// void EventBrowsingandBooking::on_ENTERPUSH_BUTTON_clicked()
// {
    ...
    , ui(new Ui::EventBrowsingandBooking)
{
    ...
    // Connect the textChanged signal of the search input field to the slot
    connect(ui->searchEventsLineEdit, &QLineEdit::textChanged, this, &EventBrowsingandBooking::on_searchTextChanged);
}
...
void EventBrowsingandBooking::loadEventsFromFile() {
    ...
    while (!in.atEnd()) {
        ...
        if (fields.size() != 6) { // Expecting 6 fields
            qWarning() << "Invalid line format, skipping:" << line;
            continue;
        }
...
void EventBrowsingandBooking::on_searchTextChanged(const QString &searchQuery)
{
    // Clear the table before populating with filtered results
    eventTable->setRowCount(0);

    // If the search field is empty, reload all events
    ...
    while (!in.atEnd()) {
        ...
        if (fields.size() != 6) { // Skip invalid lines
            qWarning() << "Invalid line format, skipping:" << line;
            continue;
        }
        ...
        double ticketPrice = fields[4].trimmed().toDouble();
        int availableSeats = fields[5].trimmed().toInt();

        // Check if the event name starts with the search query
        if (title.startsWith(searchQuery, Qt::CaseInsensitive)) {
            int row = eventTable->rowCount();
            eventTable->insertRow(row);
    ...
    file.close();
    qDebug() << "Finished filtering events.";
}*/
#include "EventBrowsingandBooking.h"
#include "ui_EventBrowsingandBooking.h"
#include <QFile>
